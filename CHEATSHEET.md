...
# 🔗 Linear + GitHub lane (optional)

Wraps the role loop so the whole trail is **Conversation → Ticket → Spec → Implementation → PR**.

| Command                     | Purpose                                                        |
| --------------------------- | ------------------------------------------------------------- |
| `/sdd-ticket`               | Turn an idea into a well-formed Linear ticket (no pasting)     |
| `/sdd-board`                | List the project's issues by status, pick one                 |
| `/sdd-task <id>`            | Load one ticket by id or title                                 |
| `/sdd-spec-from-ticket <id>`| Draft a spec from a ticket (Socratic, links the ticket)        |
| `/sdd-pr`                   | Open a draft PR for the finished spec, request review          |

### One-time setup (per person, per machine)

```bash
# Linear: the repo already ships .mcp.json. Just authorize:
#   open a Claude Code session in the repo, run:  /mcp   → complete browser OAuth
# GitHub:
gh auth login
```

Coordinates (Linear project, repo, branch/PR conventions) live in
`ai/context/integrations.md` — edit them there, not in the commands.

> Writes to Linear and opening a PR are always confirmed with you first. `git push`,
> `gh pr create`, and adding reviewers are intentionally left to prompt — only read-ish
> git/gh commands are pre-approved in `.claude/settings.json`.
