...
5. **Post the review** (`gh pr review --approve` / `--request-changes`). In HITL, show the draft and confirm before posting; in autonomous, post directly.
6. Update `ai/STATE.md` with a decision line. On request-changes, route the fix back through `/sdd-orchestrate` (Developer), then re-run `/sdd-pr-review`.
